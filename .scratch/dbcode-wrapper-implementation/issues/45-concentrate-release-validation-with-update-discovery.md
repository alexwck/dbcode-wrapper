...
Automatic polling and the update-status UI remain unchanged and read-only. Host Release packaging now performs one full validation, carries its result in one checked context, and accepts only an exact copied app. Independent mounted-DMG verification remains a full check. This removes repeated release work without weakening the public package boundary or moving publication authority into the app.
