...
The wrapper keeps New Query, Open SQL, DBCode SQL context actions, History, Library, Scratch Files, Query Builder, and DBCode settings. The DBCode `1.36.4` stored-routine debugger is a candidate-only feature until the separate real PostgreSQL debugger proof passes.

Coverage: broad declared and reachable coverage; rendered SQL, Query Builder, History, and Library; live representative query checks; debugger requires validation.
...
Direct CSV, Excel, Parquet, and Avro file opening is a known compatibility gap. DBCode declares its custom data-file editor, but the prior approved host pair did not register the provider. The wrapper keeps the guaranteed-failure shortcut hidden while the DBCode `1.36.4` candidate receives a separate direct-file check. Opening the same data through Connections is useful but is not feature-equivalent.
