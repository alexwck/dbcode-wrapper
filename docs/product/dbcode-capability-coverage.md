...
MongoDB may encode exact BSON types in that copied data as canonical Extended JSON wrappers such as `{"$numberInt":"0"}`. The wrapper's **Open Copied BSON Result** and **Open BSON Result File…** actions display explicit local input as Tree, Table, or Raw JSON. The table separates Path, Value, and Type, so the example appears as value `0` and type `Int32`; numeric strings remain type `String`. Search covers path, value, and type, while **Parse JSON strings** optionally expands JSON stored inside a string. This is an opt-in display adapter, not a live result replacement: it never runs a query, reads DBCode internals, connects to a database, watches the clipboard, or sends data over the network.
