...
3. **Compiled Host Cache** separates expensive Code OSS compilation from release assembly. Its content-addressed input covers upstream revisions, the toolchain, active Release Specification code, compile-time product values, the first-class Code OSS overlay, small upstream patches, icon, and active slimming choices. Cache integrity covers bytes, links, and executable modes. Its receipt records the actual compiler environment. DBCode package metadata, profile-only names, the profile schema, dated slimming measurements, and assembly-only wrapper files do not change that ID. Query storage names do because the focused shell embeds them in the compiled product record.
...
6. **Profile Setup** owns the first-use action order, reviewed import plan, temporary-file cleanup, DuckDB preflight, completion state, cancellation, and recovery handoff. A small command router registers Runtime Setup and Profile Setup before startup state is known, then sends Profile Setup to its required runtime prerequisite when needed. Both first-run webviews share one content-security, nonce, escaping, and action-message policy. The host adapter alone calls VS Code, DBCode, the clipboard, the filesystem, time, process spawning, and quit commands.
7. **Host Session** owns one application lifecycle: process start, renderer readiness, DBCode readiness, logs, timeout, and complete quit. The normal shell launcher passes one record containing only launch-specific values. Host Session owns the stable readiness, logging, and shutdown policy. Its command adapter exposes only `run`; the reusable JavaScript lifecycle keeps internal and public shutdown paths.
...
9. **Focused Runtime Setup** derives one public package-and-key record from the Release Specification. Its Open VSX Package Verifier owns runtime-record shape, canonical package selection, installed identity, registry identity, engine compatibility, sizes, digests, key binding, Ed25519 signatures, archive safety, signature manifests, and VSIX identity. Finder setup and scripted preparation keep separate download and private-cache adapters. They fetch files and call the verifier; they do not know the package field list or own another copy of the security rules. On a fresh Finder launch the setup downloads only the exact DBCode and Python/Jupyter set, installs verified packages outside the app with extension-pack dependencies disabled, and reloads only after the managed inventory matches.
