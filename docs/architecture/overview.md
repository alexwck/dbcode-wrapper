...
12. **Local BSON Result Viewer** owns one lossless display-model interface for explicitly supplied JSON. Its pure parser recognizes exact MongoDB Extended JSON wrappers, preserves the original raw text, keeps ordinary strings distinct, and records readable values with separate BSON or JSON types. A thin host adapter reads the clipboard or one selected file only after the matching command, enforces input and node limits, and sends the model to a nonce-protected webview. The webview uses text nodes for result data, keeps search text and raw input out of persisted state, clears its in-memory payload on close, and provides Tree, bounded Table, and Raw JSON modes. It does not inspect DBCode, connect to a database, poll, use the network, send telemetry, or write results.
