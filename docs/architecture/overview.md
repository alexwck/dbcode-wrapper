...
7. **Host Session** owns one application lifecycle: process start, renderer readiness, DBCode readiness, logs, timeout, and complete quit. Its command adapter exposes only `run`; the reusable JavaScript lifecycle keeps internal and public shutdown paths.
