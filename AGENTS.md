...
- Before `release_host.sh prepare` acquires the distribution checkpoint or starts expensive work, validate the current branch, source commit, working-tree requirement, and version-tag state. A new tag requires clean `main`; an existing tag must be annotated at the current commit so exact same-tag preparation can resume. Keep `plan` read-only and let it report the blocker.
...
- Test behaviour through the same maintained interface production uses. Do not export an implementation helper only for a test, and do not keep a test whose only job is naming a helper that remains private. Keep an exact interface test when that small interface is deliberate. Include every production adapter that shapes the signed app in wrapper source identity; exclude test-only helpers.
