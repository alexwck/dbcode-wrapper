...
description: The path from one immutable source commit through generated profile identity to a signed DBCode Wrapper session.
type: flow
tags:
  - wiki
  - flow
...
This flow creates a fresh auditable release without recompiling an unchanged host. The build records and materializes one clean source commit, reuses or creates the exact Compiled Host cache entry, assembles wrapper extensions and generated records, signs the app, and writes the manifest. Static and rendered checks then verify the exact release.

Profile-only names and schema are assembly inputs. Query storage names are compilation inputs because the focused shell embeds them in the product record.
...
sequenceDiagram
  ...
  Assemble->>Assemble: Generate profile runtime and release records
  Assemble->>Sign: Fresh complete app
  Sign-->>Verify: Signed app and manifest
  Verify->>Verify: Regenerate and compare profile identity
...
6. Copy wrapper extensions and generate profile identity, runtime package, and release-status records.
7. Sign the app and generate a manifest containing source, compiled-host, app, profile, and release identities.
8. Run static smoke. It regenerates the profile identity, compares the packaged bytes, validates Profile Layout, and checks the query-storage product fields without launching the app.
...
- A profile-only change unexpectedly invalidates the Compiled Host.
- The tracked, generated, or packaged profile identity is missing, unsafe, stale, or different.
