...
10. Transfer only through the owner's private location. When GitHub is used, keep the release as an unpublished authenticated draft, compare every uploaded size and SHA-256 digest, verify an owner download, prove anonymous denial, and confirm that no workflow can publish it.
...
The current checkpoint completed steps 1–10 for private release `v0.1.2`, with DBCode `1.36.4` on the retained host. The five uploaded host-only assets matched the local transfer set, authenticated owner download succeeded, and anonymous listing, tag lookup, and direct asset access were denied. The draft remains unpublished. The app and production profile were not changed.
