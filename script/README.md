...
- `release_host.sh plan|prepare|publish --publish` is the normal owner-facing interface. It derives the tag and standard paths from the Release Specification. `prepare` runs acceptance before creating or verifying the annotated tag, then packages, approves, and leaves one exact change in `host/approved-release-history.json`. It reuses complete exact evidence after an interrupted run. Commit the history file before the separate publication action.
