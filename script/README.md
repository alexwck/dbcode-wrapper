...
- `release_host.sh plan|prepare|publish --publish` is the normal owner-facing interface. `plan` is read-only and reports the current source plus a structured preparation state. `prepare` rejects an off-`main`, unclean new source, lightweight tag, or version tag at another commit before it acquires the kernel-backed `dist/` checkpoint lease or starts expensive work. An annotated tag at the current commit remains resumable when the only working-tree change is the expected approval-history edit. After preflight, `prepare` passes the lease to every build and verification child and owns prompt-free signing readiness, build or exact reuse, static smoke, one-profile rendered smoke, final acceptance, tagging, packaging, independent verification, and approval. Generated release evidence is keyed by both tag and immutable source commit, so retained evidence for an older use of the same tag stays untouched. A repeated run revalidates package files and approval digests before reuse, then leaves one approval-history file to commit. Publication stays separate and explicit.
