...
`plan` is read-only. It reports whether the current commit can enter preparation and explains any branch, working-tree, or version-tag blocker. `prepare` checks that state before it acquires the build checkpoint or starts signing, building, or verification. It then builds or reuses the exact Host, runs static smoke and one rendered smoke with the persistent generated `qa` profile, performs final prompt-free acceptance, creates or verifies the annotated source tag, packages and independently verifies the host-only DMG, and records approval. An existing annotated tag at the current commit supports an exact resume when the only working-tree change is the expected approval-history edit. The task holds one operating-system checkpoint lease throughout the work that follows preflight, and every child keeps that lease while it reads `dist/`.
